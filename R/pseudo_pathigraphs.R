...
create.html.report3 <- function(pseudo_pathigraphs, metaginfo, comp, home,
    ...
    # if(clean_out_folder==TRUE & file.exists(paste0(output.folder,"/report"))){
    #   unlink(paste0(output.folder,"/report"), recursive = TRUE)
    # }
            ...
            out <- paste0(out, "\"shortname\":\"",
                          gsub("\\*", "",
                               get.path.names(metaginfo,
                                              rownames(path_info[[x]])[i])),
                          "\", ")
            out <- paste0(out, "\"pvalue\":",
                          path_info[[x]]$FDRp.value[i], ", ")
            out <- paste0(out, "\"status\":\"",
                          path_info[[x]]$status[i], "\", ")
            out <- paste0(out, "\"sig\":\"",
...
#         down_subeatt <- subeatt[down_ind,,drop = FALSE]
#
#         selected_subsif <- subsif[1,]
#         selected_subsif[2] <- paste0(selected_subsif[2],".down")
#
...
#         def_sif <- rbind(def_sif,selected_subsif)
#
#         up_subepath_assoc <- subepath_assoc[up_ind,,drop = FALSE]
