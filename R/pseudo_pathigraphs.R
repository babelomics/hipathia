...
get.pseudo.pathigraphs <- function(subgraphs,annots){
  ...
  for(i in 1:length(categories)){
    ...
    pseudo_pathigraphs[[path.id]] <- create.pathigraph.from.subgraphs(selsub,categories[i],path.id)    
  }
...
create.pathigraph.from.subgraphs <- function(selsub,path.name,path.id){
  ...
  #arg <- c("hsa04014__38","hsa04014__41","hsa04014__100","hsa04014__24")
  #lapply(arg,function(x) V(selsub[[x]])$nodeY[which(V(selsub[[x]])$name=="hsa04014__21")])
  ...
  node_atts <- unique(do.call("rbind",lapply(selsub,function(x) {
    data.frame(name=V(x)$name,label=V(x)$label,shape=V(x)$shape,x=V(x)$nodeX,y=V(x)$nodeY,width=V(x)$width,height=V(x)$height,label.color=V(x)$label.color,label.cex=V(x)$label.cex,stringsAsFactors=F)
  })))
  rownames(node_atts) <- node_atts$name
  
  edges <- unique(do.call("rbind",lapply(selsub,function(x) {el <- get.edgelist(x); data.frame(source=el[,1],relation=E(x)$relation,target=el[,2])})))
  
  supergraph <- graph.data.frame(edges[,c(1,3)],directed = T)
  E(supergraph)$relation <- edges[,2]
  for(k in 1:ncol(node_atts)){
    field <- colnames(node_atts)[k]
    supergraph <- set.vertex.attribute(supergraph,name=field,value=node_atts[V(supergraph)$name,field])
  }
...
create.html.report3 <- function(pseudo_pathigraphs, metaginfo,comp, home, output.folder, effector=F,conf=0.05,extra_javascript="",after_html="",before_html="",clean_out_folder=T,template_name="index_template.html",output_name="index.html"){
  
  
  # if(clean_out_folder==T & file.exists(paste0(output.folder,"/report"))){
  #   unlink(paste0(output.folder,"/report"),recursive=T)
  # }
  # if(!file.exists(paste0(output.folder,"/report"))) dir.create(paste0(output.folder,"/report"))
  # cp_command <- paste0("cp -r ",home,"/network-viewer/* ",output.folder,"/report/")
  # system(cp_command)
  # 
  # index <- scan(paste0(output.folder,"/report/",template_name),comment.char = "",sep="\n",what="character")
  
  
  if(clean_out_folder==TRUE & file.exists(paste0(output.folder,"/pathway-viewer"))){
    unlink(paste0(output.folder,"/pathway-viewer"),recursive=TRUE)
    unlink(paste0(output.folder,"/index.html"),recursive=TRUE)
  }
  if(!file.exists(paste0(output.folder,"/pathway-viewer"))) dir.create(paste0(output.folder,"/pathway-viewer"))
  cp_command <- paste0("cp -r ",home,"/pathway-viewer/* '",output.folder,"/pathway-viewer/'")
  system(cp_command)
  mv_legend_command <- paste0("cp  ",home,"/report-files/pretty_legend.png '", output.folder,"'")
  system(mv_legend_command)
  
  index <- scan(paste0(home,'/report-files/',template_name),comment.char = "",sep="\n",what="character")
  ...
  global_div <- c(global_div,paste0("  <div id='mini_pathway_title'>Pathways list</div>"))
  global_div <- c(global_div,paste0("  <div id='pathway_title'></div>"))
  global_div <- c(global_div,paste0("  <div id='network_viewer_scroll'><div id='network_viewer'></div></div>"))
  global_div <- c(global_div,paste0("  <div id='pathway_selector'>"))
  for(pathway in names(pseudo_pathigraphs)){
    global_div <- c(global_div,paste0("  <div class='pathway_selector_item aaa' data-pathway='",pathway,"' data-pathwayname='",pseudo_pathigraphs[[pathway]]$path.name,"' onclick='handlePathwayClick(\"",pathway,"\",\"",pseudo_pathigraphs[[pathway]]$path.name,"\")'>",pseudo_pathigraphs[[pathway]]$path.name,"</div>"))
  }
  global_div <- c(global_div,paste0("  </div>"))
  global_div <- c(global_div,paste0(" <div id='mini_path_title'>Available subpaths</div>"))
  global_div <- c(global_div,paste0("  <input id='pathwayFilterInput' type='text' placeholder='Search...'/>"))
  ...
  for(pathway in names(pseudo_pathigraphs)){
    ...
    # define
    raw_sif <- read.table(paste0(output.folder,"/pathways/",pathway,".sif"),sep="\n",header=F,stringsAsFactors=F)
    sif <- paste0("sif: '",paste(raw_sif$V1,collapse="\\\\n"),"'")
    
    # node att
    raw_att <- read.table(paste0(output.folder,"/pathways/",pathway,".natt"),sep="\n",header=F,stringsAsFactors=F,comment.char = "")
    raw_att$V1 <- gsub("\\\\n"," & ",raw_att$V1)
    att <- paste0("natt: '#",paste(raw_att$V1,collapse="\\\\n"),"'")
    
    # edge att
    raw_eatt <- read.table(paste0(output.folder,"/pathways/",pathway,".eatt"),sep="\n",header=F,stringsAsFactors=F,comment.char = "")
    raw_eatt$V1 <- gsub("\\\\n"," & ",raw_eatt$V1)
    ...
    pathlist <- paste0("plist: ",paste("[",paste("\"",names(s),"\"",collapse=",",sep=""),"]"))
    prettynames <- gsub("\\*","",sapply(strsplit(get.path.names(metaginfo,names(s)),": "),"[[",2))
    prettynames <- gsub("\n"," - ",prettynames)
    prettypathlist <- paste0("pretty_plist: ",paste("[",paste("\"",prettynames,"\"",collapse=",",sep=""),"]"))
    
    pcolors <- c("UP"="darkred","DOWN"="darkblue")[comp[names(s),"UP/DOWN"]]
    pcolors[comp[names(s),"FDRp.value"]>=conf] <- "darkgrey"
    
    prettycolors <- paste0("pretty_color_plist: ",paste("[",paste("\"",pcolors,"\"",collapse=",",sep=""),"]"))
    
    
    pathstr <- paste0(pathway,":{\n",paste(sif,att,eatt,pathlist,prettypathlist,prettycolors,sep=",\n"),"\n}")
    
    global_define <- c(global_define,pathstr)
    
  }
  
  global_init <- c(global_init,paste0("this.loadPathway(\"",names(pseudo_pathigraphs)[1],"\",\"",pseudo_pathigraphs[[1]]$path.name,"\",el);"))
  
  new_index <- gsub("PUT_HERE_YOUR_ELEMENTS",paste(global_div,collapse="\n"),
                    gsub("PUT_HERE_YOUR_NETWORKS",paste(global_define,collapse=",\n"),
                         gsub("INIT_HERE_YOUR_NETWORKS",paste(global_init,collapse="\n"),index)))
...
create.pseudo.path.info <- function(all_comp, metaginfo.pseudo, metaginfo){
  ...
  path_json_list <- lapply(names(path_info),function(x){
    ...
    for(i in 1:nrow(path_info[[x]])){
      if(path_info[[x]]$has_change[i]==TRUE) anychanged <- TRUE
      if(path_info[[x]]$FDRp.value[i]<=0.05) {
        anysig <- TRUE
        if(path_info[[x]]$status[i]=="UP") anysigup <- TRUE
        if(path_info[[x]]$status[i]=="DOWN") anysigdown <- TRUE
      }
      if(path_info[[x]]$status[i]=="UP") anyup <- TRUE
      if(path_info[[x]]$status[i]=="DOWN") anydown <- TRUE
    }
    ...
    for(i in 1:nrow(path_info[[x]])){
      ...
      out <- paste0(out,"\"name\":\"", get.path.names(metaginfo, rownames(path_info[[x]])[i]),"\", ")
      # out <- paste0(out,"\"shortname\":\"",gsub("\\*","",strsplit(get.path.names(metaginfo,rownames(path_info[[x]])[i]),": ")[[1]][2]),"\", ")
      out <- paste0(out,"\"shortname\":\"",gsub("\\*","",get.path.names(metaginfo,rownames(path_info[[x]])[i])),"\", ")
      out <- paste0(out,"\"pvalue\":", path_info[[x]]$FDRp.value[i],", ")
      out <- paste0(out,"\"status\":\"", path_info[[x]]$status[i],"\", ")
      out <- paste0(out,"\"sig\":\"", tolower(path_info[[x]]$FDRp.value[i]<0.05),"\", ")
      out <- paste0(out,"\"haschanged\":", tolower(path_info[[x]]$has_change[i]),", ")
      out <- paste0(out,"\"up\":", tolower(path_info[[x]]$status[i]=="UP"),", ")
      out <- paste0(out,"\"down\":", tolower(path_info[[x]]$status[i]=="DOWN"),", ")
      out <- paste0(out,"\"upsig\":", tolower(path_info[[x]]$status[i]=="UP" & path_info[[x]]$FDRp.value[i]<0.05),", ")
      out <- paste0(out,"\"downsig\":", tolower(path_info[[x]]$status[i]=="DOWN" & path_info[[x]]$FDRp.value[i]<0.05),", ")
...
create.report3 <- function(results, comp, pseudo, metaginfo, output.folder, node.colors = NULL, conf=0.05, verbose = FALSE){
    ...
    for(pathway in names(pseudo$pathigraphs)){
        ...
        write.attributes(comp, pathway, pseudo, paste0(output.folder, "/pathways/", pathway), moreatts_pathway=moreatts[[pathway]], conf=conf)
        #write.sif.files( pathigraphs[[pathway]]$graph, nodeColor, paste0(output.folder, "/pathways/", pathway))
    }
    
    pv.path <- paste0(system.file("extdata", package="hipathia"))
    create.html.report3(pseudo$pathigraphs, metaginfo = metaginfo, comp = comp, home = "/home/mhidalgo/appl/hipathia_package/inst/extdata/", output.folder = output.folder, effector = T )
...
create.node.and.edge.attributes <- function(comp, pathway, metaginfo, moreatts_pathway=NULL,conf=0.05, effector=T,reverse_xref=NULL,exp=NULL){
    print(pathway)
    ...
    natt <- cbind(V(ig)$name, V(ig)$label, 10, V(ig)$nodeX, V(ig)$nodeY, V(ig)$stroke.color, V(ig)$shape, V(ig)$type, V(ig)$label.cex, V(ig)$label.color, V(ig)$width, V(ig)$height, sapply(V(ig)$genesList, paste, collapse=","))
    colnames(natt) <- c("ID","label","labelSize","X", "Y", "color", "shape", "type", "label.cex", "label.color", "width", "height", "genesList")
    ...
    for(i in 1:length(s)){
        ...
        path_assoc[,name] <- 1
        edges_atts <- cbind(id=eids,status=status,color=color,name=name,cname=cname,pname=pname,pvalue=comp[cname,"p.value"],adj.pvalue=comp[cname,"FDRp.value"])
    ...
    for(ue in unique_edges){
        ...
        subeatt <- eatt[indexes,,drop=F]
        subepath_assoc <- epath_assoc[indexes,,drop=F]
        subsif <- sif[indexes,,drop=F]
        
        anysig <- F
        # up regulated
        upsig <- which(subeatt[,"status"]=="UP" & as.numeric(subeatt[,"adj.pvalue"])<conf)
        if(length(upsig)>0){
            ...
            mini_subeatt <- subeatt[upsig,,drop=F]
            selected_subeatt <- mini_subeatt[1,c("id","status","color","pvalue","adj.pvalue")]
            selected_subeatt["id"] <- paste(selected_subsif,collapse="_")
            def_eatt <- rbind(def_eatt,selected_subeatt)
            
            selected_subepath_assoc <- subepath_assoc[upsig,,drop=F]
            def_epath_assoc <- rbind(def_epath_assoc,colSums(selected_subepath_assoc)>0)
            
            anysig <- T
        }
        
        # down regulated
        downsig <- which(subeatt[,"status"]=="DOWN" & as.numeric(subeatt[,"adj.pvalue"])<conf)
        if(length(downsig)>0){
            ...
            mini_subeatt <- subeatt[downsig,,drop=F]
            selected_subeatt <- mini_subeatt[1,c("id","status","color","pvalue","adj.pvalue")]
            selected_subeatt["id"] <- paste(selected_subsif,collapse="_")
            def_eatt <- rbind(def_eatt,selected_subeatt)
            
            selected_subepath_assoc <- subepath_assoc[downsig,,drop=F]
            def_epath_assoc <- rbind(def_epath_assoc,colSums(selected_subepath_assoc)>0)
        ...
        if(length(nosigs)>0){
            ...
            mini_subeatt <- subeatt[nosigs,,drop=F]
            selected_subeatt <- mini_subeatt[1,c("id","status","color")]
            def_eatt <- rbind(def_eatt,selected_subeatt)
            
            selected_subepath_assoc <- subepath_assoc[nosigs,,drop=F]
            def_epath_assoc <- rbind(def_epath_assoc,colSums(selected_subepath_assoc)>0)
    ...
    def_eatt <- as.data.frame(def_eatt,stringsAsFactors=F)
    def_epath_assoc <- as.data.frame(def_epath_assoc,stringsAsFactors=F)
    rownames(def_sif) <- NULL
    def_sif <- as.data.frame(def_sif,stringsAsFactors=F)
    
    def_eatt$head <- c("inhibited","directed")[grepl("activation",def_sif[,2])+1]
    
    def_eatt <- cbind(def_eatt,(def_epath_assoc==T)+0)
    ...
    if(length(left) > 0 ){
        if(length(left) == 1){
            ids <- paste(get.edgelist(ig)[left,1], "activation", get.edgelist(ig)[left,2], sep="_")
        }else{
            ids <- apply(get.edgelist(ig)[left,], 1, function(x){paste(x[1], "activation", x[2], sep="_")})
        }
        funejes <- as.data.frame(matrix(0, nrow=length(ids), ncol=ncol(def_eatt)), stringsAsFactors=F)
        colnames(funejes) <- colnames(def_eatt)
        rownames(funejes) <- ids
        funejes$id <- ids
        funejes$status <- "DOWN"
        funejes$color <- "darkgrey"
        if("pvalue" %in% colnames(funejes)) funejes$pvalue <- ids
        if("adj.pvalue" %in% colnames(funejes)) funejes$adj.pvalue <- "DOWN"
        ...
        if(effector == T){
            ...
            funs <- t(apply(funejes, 1, function(x){
                lastnodes <- sapply(colnames(funejes), function(col){unlist(strsplit(col, split=" - "))[2]})
        ...
        funs <- as.data.frame(funs, stringsAsFactors=F)
        sif_funs <- data.frame(V1=get.edgelist(ig)[left,1], type=rep("activation", times=length(left)), V3=get.edgelist(ig)[left,2], stringsAsFactors=F)
    ...
    if(!is.null(reverse_xref)){    
        ...
        translate_ids <- function(ids){
            if(length(ids)>0){
                ids <- setdiff(ids,"/")
                tids <- sapply(reverse_xref[ids],function(x) if(is.null(x)) return("?") else return(x))
    ...
    if(!is.null(exp)){
        ...
        get_expr_ids <- function(ids){      
            if(length(ids)>0){
                ids <- setdiff(ids,"/")
                exp_values <- sapply(ids_list[ids],function(x) if(is.null(x)) return("?") else return(exp[x,]))
