...
translate.matrix <- function(exp, species, verbose=TRUE){
  ...
  xreffile <- paste0("xref_", species, ".RData")
  if(!file.exists(system.file("extdata",xreffile,package="hipathia")))
    retrieve.pathways.info(species)
  load(system.file("extdata",xreffile,package="hipathia"))
...
#' translated.names <- get.path.names(pathways, rownames(path.vals))
#' 
  ...
  prettynames <- unlist(lapply(names, function(name){
    ...
    if(length(strname) > 3){
      newini <- labelid[which(labelid[,1] == 
                              paste("N", strname[2], strname[3], sep="-")),2]
      newend <- labelid[which(labelid[,1]==
                              paste("N", strname[2], strname[4], sep="-")),2]
...
igraphs.upgrade <- function(metaginfo){
  ...
  for(pw in names(metaginfo$pathigraphs)){
    ...
    metaginfo$pathigraphs[[pw]]$graph <- upgrade_graph(metaginfo$pathigraphs[[pw]]$graph)
    
    for(sp in names(metaginfo$pathigraphs[[pw]]$effector.subgraphs)){
      metaginfo$pathigraphs[[pw]]$effector.subgraphs[[sp]] <- 
        upgrade_graph(metaginfo$pathigraphs[[pw]]$effector.subgraphs[[sp]])
    }
    
    for(sp in names(metaginfo$pathigraphs[[pw]]$subgraphs)){
      metaginfo$pathigraphs[[pw]]$subgraphs[[sp]] <- 
        upgrade_graph(metaginfo$pathigraphs[[pw]]$subgraphs[[sp]])
    }
...
load.pathways <- function(species, pathways.list = NULL){
  metagraphfile <- paste0("meta_graph_info_", species, ".RData")
  if(!file.exists(system.file("extdata", metagraphfile, package="hipathia")))
    retrieve.pathways.info(species)
  load(system.file("extdata", metagraphfile, package="hipathia"))
...
#' @import RCurl
retrieve.pathways.info <- function(species){
  message(paste0("Retrieving pathways data for ", species))
  if(!dir.exists(system.file("extdata", package="hipathia")))
    dir.create(paste0(system.file(package="hipathia"), "/extdata"))
  url <- "http://bioinfo.cipf.es/apps-beta/hipathia/hipathia-files/0.99.0/"
  hipathia.folder <- system.file("extdata", package="hipathia")
  
  afu.file <- paste0("annofuns_uniprot_", species, ".RData")
  afg.file <- paste0("annofuns_GO_", species, ".RData")
  eh.file <- paste0("entrez_hgnc_", species, ".annot")
  gb.file <- paste0("go_bp_", species, ".annot")
  mgi.file <- paste0("meta_graph_info_", species, ".RData")
  uk.file <- paste0("uniprot_keywords_", species, "__biological_process.annot")
  x.file <- paste0("xref_", species, ".RData")
  gbn.file <- "go_bp_net.RData"
  gbf.file <- "go_bp_frame.RData"
  
  if(!file.exists(system.file("extdata", afu.file, package="hipathia")))
    utils::download.file(url = paste0(url, afu.file), 
                       destfile = paste0(hipathia.folder, "/", afu.file), 
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", afg.file, package="hipathia")))
    utils::download.file(url = paste0(url, afg.file), 
                       destfile = paste0(hipathia.folder, "/", afg.file), 
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", eh.file, package="hipathia")))
    utils::download.file(url = paste0(url, eh.file), 
                       destfile = paste0(hipathia.folder, "/", eh.file), 
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", gb.file, package="hipathia")))
    utils::download.file(url = paste0(url, gb.file), 
                       destfile = paste0(hipathia.folder, "/", gb.file), 
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", mgi.file, package="hipathia")))
    utils::download.file(url = paste0(url, mgi.file), 
                       destfile = paste0(hipathia.folder, "/", mgi.file),
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", uk.file, package="hipathia")))
    utils::download.file(url = paste0(url, uk.file), 
                       destfile = paste0(hipathia.folder, "/", uk.file),
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", x.file, package="hipathia")))
    utils::download.file(url = paste0(url, x.file), 
                       destfile = paste0(hipathia.folder, "/", x.file), 
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", gbn.file, package="hipathia")))
    utils::download.file(url = paste0(url, gbn.file), 
                       destfile = paste0(hipathia.folder, "/", gbn.file), 
                       method = "libcurl", quiet = TRUE)
  if(!file.exists(system.file("extdata", gbf.file, package="hipathia")))
    utils::download.file(url = paste0(url, gbf.file), 
                       destfile = paste0(hipathia.folder, "/", gbf.file), 
                       method = "libcurl", quiet = TRUE)
}
...
add.missing.genes <- function(exp.data, genes, default=NULL){
  ...
  if(length(missing_genes>0)){
    if(ncol(exp.data)==1){
      fakemat <- default + as.matrix(mat.or.vec(
        nr=length(missing_genes),nc=ncol(exp.data)))
    } else {
    ...
    message("----------------------------------------------------")
    message("Added missing genes: ", length(missing_genes), " (", 
...
#' @export
#'
get.pathways.annotations <- function(pathway.names, metaginfo, dbannot, collapse = TRUE){
  ...
  if(is.character(dbannot)){
    ...
    eh.sys.file <- system.file("extdata", eh.file, package="hipathia")
    entrez2hgnc <- utils::read.table(eh.sys.file, header=FALSE, 
...
#' #@export
#' 
get.highest.sig.ancestor <- function(go_terms, go_comp, metaginfo, unique = TRUE, pval = 0.05){
  
    if(!file.exists(system.file("extdata", "go_bp_frame.RData", package="hipathia")) | 
       !file.exists(system.file("extdata", "go_bp_net.RData", package="hipathia")))
        retrieve.pathways.info(metaginfo$species)
    load(system.file("extdata", "go_bp_frame.RData", package="hipathia"))
    load(system.file("extdata", "go_bp_net.RData", package="hipathia"))
        ...
        if(length(sig.ancestors) > 0 ){
            ...
            if(unique == TRUE){
                highest.ancestors <- go_bp_frame[sig.ancestors, 
                                             "id"][sig.levels == min(sig.levels)][1]
            }else{
                highest.ancestors <- go_bp_frame[sig.ancestors, 
                                             "id"][sig.levels == min(sig.levels)]
            }
            df <- data.frame(GO_term = label, 
                             ...
                             HSA_adj_pval = go_comp[highest.ancestors, "FDRp.value"],
                             stringsAsFactors = FALSE)
        }else{
          df <- data.frame(GO_term = label, 
                           GO_name = go_bp_frame[label, "name"], 
...
#' comp.go <- do.wilcoxon(go.values, sample.group, g1 = "Tumor", g2 = "Normal")
...
#' results <- hipathia(exp.data, pathways, verbose=FALSE)
#' path.vals <- get.paths.matrix(results)
#' path.normalized <- normalize.paths(path.vals, pathways)
#' 
#' @export
#' 
normalize.paths <- function(path.vals, metaginfo){
    decomposed <- !all(sapply(sapply(rownames(path.vals), strsplit, "-"), length) == 3)
    if(decomposed == TRUE){
        norm.factors <- metaginfo$path.norm[rownames(path.vals)]
        path.norm <- normalize.data(path.vals/(norm.factors*0.99+0.01), 
                                    by.quantiles = FALSE, by.gene = FALSE, percentil = FALSE)
    }else{
