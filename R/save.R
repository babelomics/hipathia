...
#' path.vals <- get.paths.matrix(results)#' pathways <- load.pathways(species = "hsa")
...
save.results <- function(results, comp, metaginfo, output.folder){
  ...
  utils::write.table(comp, file=paste0(output.folder,"/all_path_stats.txt"), 
                     col.names=TRUE, row.names=TRUE, quote=FALSE, sep="\t")
  ...
  if(!is.null( results$all$accuracy )){
    ...
    utils::write.table(accu, file=paste0(output.folder,"/accuracy.txt"), 
                       col.names=TRUE, row.names=TRUE, quote=FALSE, sep="\t")
  }
...
write.attributes <- function(this_comp, pathway, metaginfo, prefix, moreatts_pathway=NULL, conf=0.05, reverse_xref=NULL, exp=NULL){
...
create.node.and.edge.attributes <- function(comp, pathway, metaginfo, moreatts_pathway=NULL, conf=0.05, reverse_xref=NULL, exp=NULL){
  ...
  V(ig)$tooltip <- sapply(1:length(V(ig)), function(i){
    if(V(ig)$shape[i] == "ellipse"){
      paste(sapply(V(ig)$genesList[[i]], function(gen){
        if(!gen == "/"){
          paste0("<a target='_blank' href='http://www.genome.jp/dbget-bin/www_bget?", 
  ...
  if(length(left) > 0 ){
    ...
    funejes <- as.data.frame(matrix(0, nrow=length(ids), ncol=ncol(def_eatt)),
...
create.html.report2 <- function(pathigraphs, comp, home, output.folder, conf=0.05,extra_javascript="",after_html="",before_html="",clean_out_folder=TRUE,template_name="index_template.html",output_name="index.html"){
  ...
  if(!file.exists(pv.folder)) dir.create(pv.folder)
  file.copy(paste0(home,"/pathway-viewer/"), pv.folder, recursive=TRUE)
  ...
  global_div <- c(global_div,paste0("<pathway-viewer id='pathway-viewer' path-type='url' path='pathways'></pathway-viewer>"))
...
#' create.report(results, comp, pathways, "~/save_results/", node.colors = colors.de)
#' 
...
#' @export
#' 
create.report <- function(results, comp, metaginfo, output.folder, node.colors = NULL, conf=0.05, verbose = FALSE){
