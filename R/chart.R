...
#'
heatmap.plot <- function(path.vals, sample.type = NULL, colors="classic", sample.clust=TRUE, variable.clust=FALSE, labRow=NULL, labCol=NULL, sample.colors=NULL, scale=TRUE, save.png=NULL, legend = TRUE, legend.xy = "topright", pch=15, main=NULL){  
  if(length(colors) == 1){
    if(colors == "hipathia"){ colors <- c("#007462", "white", "#e66430")
    }else if(colors == "classic"){ colors <- c("blue","gray","red")
    }else if(colors == "redgreen"){ colors <- c("green","black","red")}
  }
  if(is.null(sample.type)) sample.type <- rep("A", ncol(path.vals))
  ...
  if(is.null(sample.colors)){
    if(length(unique(sample.type)) <= 8){
      sample.colors <- c("#50b7ae", "#b6ebe7", "#e66430", 
                         ...
                         "#a0170e", "#f9b493")[1:length(unique(sample.type))]
    }else{
      sample.colors <- c("#50b7ae", "#b6ebe7", "#e66430", "#305f59", 
                         "#ffc868", "#152e2b", "#a0170e", "#f9b493", 
                         grDevices::terrain.colors(length(unique(sample.type))-8))
    }
...
#' path.vals <- get.paths.matrix(results)
#' sample.group <- brca_design[colnames(path.vals),"group"]
#' pca.model <- do.pca(path.vals[1:ncol(path.vals),])
#' pca.plot(pca.model, sample.group)
#'
#' @export
#' @import grDevices graphics
#'
pca.plot <- function(fit, sample.type = NULL, sample.colors=NULL, cp1=1, cp2=2, legend = TRUE, legend.xy = "bottomleft", cex=2, pch=20, mgp = c(3,1,0), main="PCA plot", save.png=NULL){
  ...
  if(is.null(sample.colors)){
    if(length(unique(sample.type)) <= 8){
      ...
      sample.colors <- c("#50b7ae", "#b6ebe7", "#e66430", "#305f59", 
                         "#ffc868", "#152e2b", "#a0170e", "#f9b493", 
                         grDevices::topo.colors(length(unique(sample.type))-8))
    }
    names(sample.colors) <- unique(sample.type)
  }
  cpv1 <- fit$scores[,cp1]
...
#'
multiple.pca.plot <- function(fit, sample.type = NULL, sample.colors=NULL, comps=1:3, plot.variance = FALSE, legend = TRUE, cex=2, pch=20, main = "Multiple PCA plot", save.png=NULL){
  combs <- utils::combn(comps,2)
  ncombs <- ncol(combs)
  nn <- ncombs
  if(!is.null(legend)) nn <- nn + 1
  if(plot.variance==TRUE) nn <- nn + 1  
...
plot.pathigraph <- function(g, node.color=NULL, edge.lty=1, main="" ){
  ...
  V(g)$size2 <- 5
  if(is.null(node.color)){V(g)$color <- "white"}else{V(g)$color <- node.color}
  E(g)$lty <- edge.lty
  plot.igraph(g, 
...
#' @export
#'
pathway.comparison.plot <- function(comp, metaginfo, pathway, conf=0.05, node.colors = NULL, colors = "classic"){
...
add.edge.colors <- function(pathigraph, pcomp, effector, up.col = "#ca0020", down.col = "#0571b0", no.col = "darkgrey", conf = 0.05){
...
#' @export
#' 
node.color.per.differential.expression <- function(results, metaginfo, groups, group1.label, group2.label, colors = "classic", conf = 0.05){ 
  
  if(length(colors) == 1){
    if(colors == "hipathia"){ colors <- c("#50b7ae", "white", "#f16a34")
    }else if(colors == "classic"){ colors <- c("#1f9cda","white","#da1f1f")}
  }
  down.col <- colors[1]
  no.col <- colors[2]
  up.col <- colors[3]
  ...
  cols <- lapply(names(results$by.path), function(path){
    difexp.nod <- compute.difexp(results$by.path[[path]]$nodes.vals, 
                                 group1.label, 
                                 group2.label, 
                                 groups)
    updown <- sapply(difexp.nod$statistic, function(x){
      if(x <0){"down"}else if(x > 0){"up"}else{"both"}
      })
    col <- get.colors.from.pval(updown, 
                                difexp.nod$p.value, 
                                up.col = up.col, 
                                down.col = down.col, 
                                no.col = no.col, 
    ...
    # Add function colors
    pathgraph <- metaginfo$pathigraphs[[path]]$graph
    toadd <- V(pathgraph)$name[!V(pathgraph)$name %in% rownames(difexp.nod)]
