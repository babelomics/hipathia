...
#' results <- hipathia(exp.data, pathways, decompose = TRUE, verbose = FALSE)
#' 
#' @export
#' 
hipathia <- function(genes.vals, metaginfo, decompose = FALSE, maxnum = 100, verbose = TRUE, tol = 0.000001){
...
nodes.values.from.genes <- function(genes.vals, ig, summ="per90"){
  ...
  nodes.vals <- matrix(NA, 
                       ...
                       dimnames=list(names(genes.list), colnames(genes.vals)))
  for (node.name in names(genes.list)){
    ...
    if( "/" %in% genes ){ #Then the node is a protein complex
      ...
      for( list1 in lists ){
        if( length(list1) > 1 ){
          prob <- summarize.probabilities(genes.vals[list1,,drop=FALSE], summ)
        }else{
      ...
      if (length(glist) > 1){
        nodes.vals[node.name,] <- 
          summarize.probabilities(genes.vals[glist,,drop=FALSE], summ)
      }else if (length(glist) == 1 && !is.na(glist)){
        nodes.vals[node.name,] <- data.matrix(genes.vals[glist,,drop=FALSE])
      }else{
...
all.path.values <- function( nodes.vals, subgraph.list, method="maxmin", maxnum=100, tol = 0.000001, divide = FALSE, response.tol = 0 ){
  path.vals <- matrix(0, 
...
path.value <- function( nodes.vals, subgraph, ininodes, endnode, method="maxmin", maxnum = 100, tol = 0.000001, divide=FALSE, response.tol = 0 ){
...
compute.node.signal <- function(actnode, node.val, node.signal, subgraph, method="maxmin", response.tol = 0){
