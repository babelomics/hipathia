...
#' exp.data <- normalize.data(trans.data, by.quantiles = TRUE, truncation.percentil=0.95)
#' 
#' @export
...
#' 
normalize.data <- function(exp.data, by.quantiles=FALSE, by.gene=FALSE, percentil=FALSE, truncation.percentil=NULL){
  ...
  if(!is.null(truncation.percentil)){
    if( truncation.percentil >= 0 & truncation.percentil <= 1){
      ...
      norm.data <- t(apply(norm.data, 1, function(x){
        quan.inf <- stats::quantile(x, 1-truncation.percentil, na.rm=TRUE)
        x[which(x < quan.inf)] <- quan.inf
        quan.sup <- stats::quantile(x, truncation.percentil, na.rm=TRUE)
...
#' @export
#' 
do.wilcoxon <- function(sel.vals, group.value, g1, g2, paired=FALSE, adjust=TRUE){
...
calculate.wilcox.test <- function( data, control, disease, paired, adjust=TRUE){
  if(paired == TRUE){
    ...
    testData <- do.call("rbind", 
                        apply(data, 1, wilcox.test.fun, control, disease, paired))
    if(adjust==TRUE){
      fdrData <- stats::p.adjust(testData[,1], method = "fdr")
    } else {
      fdrData <- testData[,1]
    ...
    need0 <- which(data2$pvalue==1 & data2$class == "0" & data2$fdrData == 1)
    data2[need0, "z"] <- 0
  }
...
wilcoxsign.test.fun <- function(x, control, disease){
  r <- try(coin::wilcoxsign_test(as.numeric(x[disease])~as.numeric(x[control]),
                                  showWarnings = FALSE))
...
get.pathways.summary <- function(comp, metaginfo, conf = 0.05){
  ...
  summ <- lapply(id.pathways, function(pathway){
    ...
    is.down.path <- minicomp$FDRp.value < conf & minicomp$`UP/DOWN` == "DOWN"
    num.down.paths <- sum(is.down.path)
    percent.down.paths <- round(num.down.paths/nrow(minicomp)*100, digits = 2)
