...
create.html.report3 <- function(pseudo_pathigraphs, metaginfo, comp, home,
    ...
    for(pathway in names(pseudo_pathigraphs)){
        global_div <- c(global_div,
                        paste0("  <div class='pathway_selector_item",
                               " aaa' data-pathway='", pathway,
                               "' data-pathwayname='",
    ...
    for(pathway in names(pseudo_pathigraphs)){
        ...
        # node att
        raw_att <- read.table(paste0(output.folder, "/pathways/", pathway,
                                     ".natt"), sep = "\n", header = FALSE,
        ...
        # edge att
        raw_eatt <- read.table(paste0(output.folder, "/pathways/", pathway,
                                      ".eatt"), sep = "\n", header = FALSE,
                               stringsAsFactors = FALSE, comment.char = "")
        raw_eatt$V1 <- gsub("\\\\n", " & ", raw_eatt$V1)
        ...
        pathlist <- paste0("plist: ", paste("[", paste("\"", names(s),  "\"",
                                                       collapse = ",",
                                                       sep = ""),
                                            "]"))
        prettynames <- gsub("\\*", "", sapply(strsplit(get.path.names(metaginfo,
                                                                      names(s)),
                                                       ": "), "[[", 2))
        prettynames <- gsub("\n", " - ", prettynames)
        prettypathlist <- paste0("pretty_plist: ", paste("[",
                                                         paste("\"",
                                                               prettynames,
                                                               "\"",
                                                               collapse = ",",
        ...
        pathstr <- paste0(pathway, ":{\n", paste(sif, att, eatt, pathlist,
                                                 prettypathlist, prettycolors,
                                                 sep = ",\n"), "\n}")
    ...
    global_init <- c(global_init,
                     paste0("this.loadPathway(\"",
                            names(pseudo_pathigraphs)[1], "\",\"",
                            pseudo_pathigraphs[[1]]$path.name, "\",el);"))
