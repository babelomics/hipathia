...
```{r, fig.show='hold'}
exp.data <- normalize.data(trans.data, percentil = TRUE)
boxplot(exp.data)
```
...
```{r, fig.show='hold'}
exp.data <- normalize.data(trans.data, truncation.percentil = 0.95)
boxplot(exp.data)
```
...
```{r, fig.show='hold'}
pathways2 <- load.pathways(species = "hsa", pathways.list = c("hsa03320", "hsa04014"))
```
...
```{r, fig.show='hold'}
length(get.pathways.list(pathways2))
get.pathways.list(pathways2)
```
...
```{r, fig.show='hold'}
results <- hipathia(exp.data, pathways, decompose = FALSE, verbose=FALSE)
```
...
```{r, fig.show='hold'}
path.names <- get.path.names(pathways, rownames(path.vals))
head(path.names)
```
...
tab <- t(sapply(c("hsa", "mmu", "rno"), function(species){
   ...
   effs <- sum(sapply(p$pathigraphs, function(pathi) length(pathi$effector.subgraphs)))
...
heatmap.plot(ranked.path.vals[1:15,], sample.group, colors="redgreen", variable.clust = TRUE)
```
...
```{r, fig.show='hold'}
pca.plot(pca.model, sample.group, legend = TRUE)
pca.plot(pca.model, sample.type = rep(1:5, 8), main = "Random types", legend = TRUE)
```
...
```{r, fig.show='hold'}
multiple.pca.plot(pca.model, sample.group, plot.variance = FALSE, legend = FALSE)
multiple.pca.plot(pca.model, sample.group, cex=3, plot.variance = TRUE)
multiple.pca.plot(pca.model, sample.type = rep(1:5, 8), main = "Random sample types")
```
...
create.report(results, comp, pathways, "save_colors", node.colors = colors.de)
```
